...
zeka = st.selectbox("Zeka (Soyutlama)", ["soyutlamasının bozuk olduğu", 
"soyutlamasının olağan olduğu"])

# ↓ Ruhsal durum metni oluştur
mse = f"İlgilinin {rapor_tarihi.strftime('%d/%m/%Y')} tarihindeki ruhsal 
durum muayenesinde duygudurumunun {duygudurum}, duygulanımının 
{duygulanim} olduğu"
...
algılamasının doğal olduğu"
    ...
    mse += ", " + ", ".join(varsanilar) + " varsanılarının olduğu, düşünce 
içeriğinin sanrısal içeriğe sahip olmadığı"
else:
...
if karar == "Yatış gerekli (kendisi ve toplum için tehlike)":
    sonuc = f"İlgiliye {tani} tanısı koyulmuş olup, kişinin söz konusu 
akıl hastalığı nedeniyle kendisi ve toplum açısından tehlikelilik 
oluşacağından TMK'nin 432. maddesi uyarınca tıbbi durumu elverir vermez 
çıkarılmak üzere kapalı psikiyatri servisi bulunan bir sağlık kurumunda 
yatarak tedavi görmesinin uygun olduğunu bildirir sağlık kurulu 
raporudur."

elif karar == "Herhangi bir bozukluk yok":
    sonuc = "Alınan öykü, incelenen evrak ve yapılan muayene sonucunda 
ilgilinin TMK'nın 432. maddesi uyarınca hastanede yatmasını gerektirecek 
nitelikte herhangi bir akıl hastalığı, akıl zayıflığı, alkol ya da madde 
bağımlılığı saptanmadığını bildirir rapordur."

elif karar == "Mental retardasyon var, yatış gerekmez":
    sonuc = "Alınan öykü, incelenen evrak ve yapılan muayene sonucunda 
ilgiliye mental retardasyon tanısı konulmuş olup bu tanının bir akıl 
zayıflığı olmakla birlikte ilgilinin halihazırdaki durumuyla toplum için 
tehlike oluşturmadığı, tedavi, eğitim veya ıslahının mümkün olmadığı, 
haliyle bir kuruma yerleştirilmesine veya alıkonulmasına gerek olmadığı 
kanaatini bildirir sağlık kurulu raporudur."

else:
    sonuc = f"Alınan öykü, incelenen evrak ve yapılan muayene sonucunda 
ilgiliye {tani} tanısı konulmuş olup bu tanının bir akıl hastalığı olmakla 
birlikte ilgilinin halihazırdaki durumuyla toplum için tehlike 
oluşturmadığı, tedavi, eğitim veya ıslahı için bir kuruma 
yerleştirilmesine veya alıkonulmasına gerek olmadığı kanaatini bildirir 
sağlık kurulu raporudur."

# Tüm metni birleştir
tam_rapor = f"""{kurum}un {ust_yazi_tarihi.strftime('%d/%m/%Y')} tarih ve 
{ust_yazi_sayisi} sayılı yazısı ile TMK’nın 432. maddesi uyarınca hakkında 
rapor düzenlenmesi için yönlendirilen {tc} T.C. kimlik nolu {ad_soyad}, 
{rapor_tarihi.strftime('%d/%m/%Y')} tarihinde Hitit Üniversitesi Erol 
Olçok Eğitim ve Araştırma Hastanesi Psikiyatri Heyetinde 
değerlendirilmiştir.
